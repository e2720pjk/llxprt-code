...
describe('Provider alias integration', () => {
  describe('Development environment tests', () => {
    ...
    it('registers user-defined alias providers from ~/.llxprt/providers', () => {
      ...
      const aliasProvider =
        providerManager.getProviderByName('myotherprovider');
    ...
    it('includes packaged provider aliases by default (development environment)', () => {
      const providerManager = getProviderManager();
  ...
  describe('Environment validation', () => {
    it('should have access to source aliases in development', () => {
      // Verify source aliases directory exists in development
      ...
      // Verify at least some alias files exist
      const aliasFiles = fs
        .readdirSync(sourceAliasesDir)
        .filter((f) => f.endsWith('.config'));
      expect(aliasFiles.length).toBeGreaterThan(0);
      expect(aliasFiles).toContain('xai.config');
      expect(aliasFiles).toContain('fireworks.config');
    });
  });
});
